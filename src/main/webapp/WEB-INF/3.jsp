...
<%@ include file="base.jsp" %>
	    	<div class="content">
	    		<h3 style="text-align:center;line-height:80px;">可选课程列表</h3>
	    		<table border=1 cellspacing=0 style="margin-left:100px;">
					<tr><td>课程编号</td><td>课程名</td><td>授课老师</td><td>职称</td><td>添加课程</td></tr>
					<% 
						
						ArrayList<Course> list = CourseService.showselectCourse(stu);
						if(list!=null){
							for(Course c:list){
								/* out.println("<p>"+teacher+"</p>"); */
								%>
								<tr><td><%=c.getCid() %></td><td><%=c.getCname() %></td><td><%=c.getTname() %></td><td><%=c.getTjob() %></td>
								<td><a href="${pageContext.request.contextPath }/servlet/selectCourse?id=<%=c.getCid() %>">添加到我的课表</a></td></tr>
								<%
							}
						}
					%>
